...
class CompanyOfficialDocumentsGenerator
{
    ...
    private function write(Fpdi $pdf,float $x,float $y,float $w,float $h,string $text,float $size=9,string $align='L',string $style=''): void { $text=$this->encode($text);for($s=$size;$s>=4.5;$s-=.5){$pdf->SetFont('Arial',$style,$s);$lineWidths=array_map(fn($line)=>$pdf->GetStringWidth($line),explode("\n",$text));if(max($lineWidths ?: [0])<=$w-1){$pdf->SetTextColor(0,0,0);$pdf->SetXY($x+.5,$y);$pdf->MultiCell($w-1,$h/ max(1,substr_count($text,"\n")+1),$text,0,$align);return;}}throw new RuntimeException('Um dado ultrapassa o limite físico do modelo. Reduza apenas abreviações de endereço ou cargo e tente novamente.'); }
